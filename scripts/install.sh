#!/usr/bin/env bash
# YAYTD — interactive full native Linux installer
...
usage() {
  cat <<'EOF'
YAYTD installer — Debian/Ubuntu native setup
...
run_wizard() {
  bold ""
  bold "  YAYTD — Yet Another YouTube Downloader"
...
# YAYTD — Yet Another YouTube Downloader
...
[Unit]
Description=YAYTD — Yet Another YouTube Downloader (Next.js)
...
[Unit]
Description=YAYTD — download worker (Redis / BullMQ / yt-dlp)
...
install_nginx_site() {
  ...
  cat > "${site}" <<EOF
# YAYTD — generated by scripts/install.sh
...
print_finish() {
  echo ""
  bold "=============================================="
  bold "  YAYTD installation complete"
  bold "=============================================="
  echo ""
  echo "  Public URL:       ${APP_URL}"
  echo "  Domain:           ${DOMAIN}"
